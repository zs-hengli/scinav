...
def chat_query_old(user_id, validated_data, openapi_key_id=None):
...
def chat_query(user_id, validated_data, openapi_key_id=None):
    ...
    try:
        resp = RagConversation.query_new(user_id, conversation_id, content)
    ...
    try:
        ...
        for line in resp.iter_lines():
            if line:
                ...
                if line_data and line_data.get('event'):
                    stream = RagConversation.update_stream(stream, line_data)
        ...
        member_ul = MemberUsageLog.objects.create(
            ...
            model=model,
            type=MemberUsageLog.UType.CHAT,
    ...
    try:
        if not conversation.is_named and stream['chunk']:
            conversation.title = RagConversation.generate_conversation_title(content, ''.join(stream['chunk']))
